                ...
                if (FD_ISSET(sockfd_icmp, &fd))
                {
                    ...
                    if (recvd_ip->protocol == IPPROTO_ICMP)
                    {
                        ...
                        if (recvd_icmp->type == ICMP_DEST_UNREACH && recvd_icmp->code == ICMP_PORT_UNREACH)
                        {
                            ...
                            { // if the source IP address of the ICMP Destination Unreachable Message does not match with your target server IP address
                        ...
                        else if (recvd_icmp->type == ICMP_TIME_EXCEEDED && recvd_icmp->code == ICMP_EXC_TTL)
                        { // if the icmp packet is for the time exceeded
                            gettimeofday(&stop, NULL);
                            print(ttl, &from_addr, &start, &stop);
                            done = 1;
                            break;
                    ...
                    { // if the packet is not an icmp packet
                        gettimeofday(&curr, NULL);
                        struct timeval diff;
                        timersub(&curr, &start, &diff);
                        int time_diff = diff.tv_sec * 1000000 + diff.tv_usec;
